...
class BidangC_M extends CI_Model
{
	...
	public function update($sertifikat,$dokumentasi,$proposal,$laporan,$materi)
	{
		...
		$this->laporan = $laporan;
		$this->materi = $materi;
		$sql = "INSERT INTO t_aktifitas values ('$max_id','1','$max_id_bidang','3','$post[kode_dosen]',NOW(),'update')";
		$this->db->query($sql);
		return $this->db->update($this->_table, $this, array('id_bidang_c' => $post['id_bidangC']));
	}
